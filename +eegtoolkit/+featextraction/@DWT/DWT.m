classdef DWT < eegtoolkit.featextraction.FeatureExtractionBase
    ...
    methods (Access = public)
        ...
        function extract(DWT)
            ...
            for i=1:numTrials
                ...
                [a4,d4] = dwt(a3,DWT.wavFamily);
                instances(i,1) = mean(abs(d2));
                instances(i,2) = std(d2);
                instances(i,3) = mean(abs(d3));
                instances(i,4) = std(d3);
                instances(i,5) = mean(abs(d4));
                instances(i,6) = std(d4);
                labels(i) = DWT.trials{i}.label;
            end
            DWT.instanceSet = eegtoolkit.util.InstanceSet(instances,labels);
        end
