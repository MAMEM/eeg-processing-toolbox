...
classdef LIBSVM < ssveptoolkit.classification.ClassifierBase
...
%         KERNEL_SIGMOID = 3; not supported yet
    ...
    methods (Access = public)
        ...
        function configInfo = getConfigInfo(LSVM)
            ...
            switch LSVM.kernel
                case LSVM.KERNEL_LINEAR
                    configInfo = sprintf('LIBSVM\tkernel:linear\tcost:%d', LSVM.cost);
                case LSVM.KERNEL_RBF
                    configInfo = sprintf('LIBSVM\tkernel:rbf\tcost:%d\tgamma:%d', LSVM.cost, LSVM.gamma);
                otherwise 
        ...
        function newLSVM = copy(LSVM)
            newLSVM = ssveptoolkit.classification.LIBSVMClassifier;
