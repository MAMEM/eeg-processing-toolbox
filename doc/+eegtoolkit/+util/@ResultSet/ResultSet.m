...
classdef ResultSet < eegtoolkit.util.InstanceSet
    ...
    methods
        function RS = ResultSet(instanceSet, labels, probabilities, ranking)
            ...
            RS = RS@eegtoolkit.util.InstanceSet(instanceSet);
        ...
        function RSSubset = subset(RS,indices)
            ...
            if isempty(RS.outputProbabilities)
                RSSubset = eegtoolkit.util.ResultSet(sInstanceSet,sOutputLabels,sOutputProbabilities);
            else
                sOutputRanking = RS.outputRanking(indices);
                RSSubset = eegtoolkit.util.ResultSet(sInstanceSet,sOutputLabels,sOutputProbabilities,sOutputRanking);
            end
        end
